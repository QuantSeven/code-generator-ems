		...
		<div class="pageFormContent form-area" title='<spring:message code="common.txt.title.info"/>'>
			<ul>
				...
				<li>
					...
					<input type="text" name="${column.columnNameLowerCase}" id="${column.columnNameLowerCase}" value="<#noparse>${</#noparse>${classNameLowerCase}.${column.columnNameLowerCase}<#noparse>}</#noparse>"  validate="{required:true}" <c:if test="<#noparse>${</#noparse>not empty ${classNameLowerCase}<#noparse>}</#noparse>"> readonly="readonly" class="readonly"</c:if> />
				</li>
